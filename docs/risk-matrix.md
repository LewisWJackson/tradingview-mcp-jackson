...
|--|--|--|--|--|--|
| 1 | **Learning overfit** (weight-adjuster kontrolsüz öğrenir, rejim değişince ters güven) | Yüksek | Portföy erimesi | Walk-forward (200 train / 50 validation), weight değişim hızı %20/gün cap, min 30 outcome threshold, **rejim-aware** ayrı weight set'leri | Shadow-mode 4 hafta boyunca weight trajectory log + out-of-sample WR karşılaştırması |
...
| 3 | **Kill switch başarısızlığı** (acil durdurma çalışmıyor) | Düşük | Felaket | Üç katmanlı: (a) API endpoint `/api/emergency/halt`, (b) terminal script `scripts/kill-all.sh`, (c) exchange-native "cancel all open orders" + pozisyonları market ile kapat | Haftalık chaos drill; paper ortamında ateşle, pozisyonlar 60sn içinde kapanıyor mu? |
| 4 | **Rejim yanlış teşhis / ping-pong** (geçişte dakikada birkaç değişim) | Orta | Ters strateji tetikler | N=3 bar histerezis, shadow-mode 2 hafta, rejim değişim rate-limit (<4 geçiş/gün/sembol), **geçiş sırasında yeni pozisyon yok** (geçiş tamamlansın) | Shadow log'dan false-flip oranı; >10% ise histerezis artır |
| 5 | **Parser kırılması** (KhanSaab/SMC format değişikliği sessiz null döner) | Yüksek | Sessiz yanlış sinyal | Her parse'ta schema validation (zorunlu field listesi), missing field → sinyal üretme + `parser_alarm` log | Günlük log review; `parser_alarm` > 0 ise acil fix |
...
|--|--|--|
| **Veri bütünlüğü** | 5, 6, 12, 13 | Parser validation, cache freshness, feed heartbeat, clock sync |
