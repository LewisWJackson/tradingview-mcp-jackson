...
- **Versiyon**: 2.5 (Faz 2 wrapper live'da, kalibrasyon penceresi)
...
| 18 | **calcADX matematik hatası** (Wilder RMA yanlış uygulandı, ADX 14× şişiyor — sürekli "trending" sahte sinyali) | Yüksek | Shadow rejim verisi tamamen yanlış (her sembol her zaman trending), volatility regime → SL multiplier hatalı (ATR×3.5 her yerde), oylama momentum oyları haksız yüksek | `calculators.js::wilderRMA` formül düzeltildi ✅ (2026-04-25): başlangıç toplam → ortalama, recurrence `out[i] = (out[i-1]*(p-1) + arr[i])/p`. Sanity test 5/5 (`scanner/tests/calc-adx.test.mjs`): ADX 0-100 sınırı + sideways<25 + trend>20 + sabit bar null | Test set CI'da koşulmalı; canlıda dashboard ADX değerleri 0-100 aralığında olmalı (sürekli >100 görüyorsan regresyon) |
...
| 2026-04-27 | 2.5 | **Faz 2 wrapper LIVE'a geçti** — 2026-04-27 21:56 UTC. Shadow log doğrulaması: v2.4 fix sonrası dağılım (PASS=2 DOTUSDC A@60 + B@240, REJECT_GATE_HTF=8, REJECT_GATE_GRADE=2, REJECT_WARMUP=11, REJECT_TRANSITION=0). Gate'ler beklendiği gibi filtreliyor. `POST /api/wrapper/mode {mode:"live", confirm:"I_REVIEWED_SHADOW_LOG"}`. Kalibrasyon penceresi başladı (1 hafta canlı veri → REGIME_GATES eşik fine-tune + weight-adjuster 6-rejim geçişi). Bilinen kısıt: 144 (symbol,tf)'den 36'sı hâlâ stableBars<3 (shadow log'da 3 ardışık aynı rejim verisi yok); gelecek scan cycle'larında dolacak. | Claude + Operatör |
| 2026-04-27 | 2.4 | **Wrapper state persist + REJECT etiket fix** — Operatör shadow log incelemesinde 54/54 kararın `REJECT_TRANSITION` etiketiyle çıktığını fark etti, live'a geçişi durdurdu. Kök neden iki katmanda: (a) `compute-regime.js` `_state` Map'i in-memory; her process restart sonrası tüm (symbol,tf) çiftleri stableBars=1'den başlıyor, 3-cycle (~3-4h) warmup boyunca `newPositionAllowed=false` dönüyor; (b) `regime-strategy.js` `newPositionAllowed=false` dalında etiketi `REJECT_TRANSITION` fallback'iyle yazıyordu, ranging/trending warmup'ları "geçiş" gibi görünüyordu, debug imkansız. Çözüm: (1) `compute-regime.js` state'i `scanner/data/regime-state.json`'a debounced flush (500ms) + `process.on('exit')` hook + atomic tmp+rename; (2) `regime-strategy.js` etiket önceliği yeniden sıralandı (`unstable→UNSTABLE`, `stableBars<3→WARMUP`, `transitioned→TRANSITION`, fallback `TRANSITION`); (3) `scanner-engine.js` `regimeContext`'e `unstable/stableBars/transitioned` alanları eklendi; (4) `scripts/seed-regime-state.mjs` shadow log'lardan state hidrate eden bootstrap (bugünkü tek seferlik kullanım: 144 entry/108 warmup-tamam). Operasyonel keşif: launchd `com.ugurtabak.tvscanner` servisi scanner'ı SIGTERM sonrası anında respawn ediyor → seed yazımı sırasında respawn varsa state üzerine yazılıyor; doğru sıra: `launchctl unload → seed → load`. Test 33/33 (regime-strategy + compute-regime). Live'a geçiş bu fix'lerle mümkün oldu. | Claude + Operatör |
| 2026-04-26 | 2.3 | **Formasyon detayları sinyal kartlarında** — operatör isteği. Her formasyon objesine `points: [{role, time, price}]` array'i eklendi (`formation-detector.js`): Çift Tepe (top1/top2/neckline/target), Çift Dip, Yükselen/Alçalan Üçgen (resistance_N/support_N), OBO/Ters OBO (left_shoulder/head/right_shoulder/neckline), Bull/Bear Flag (pole_start/pole_end/flag_end). `formatBarTime(unix)` helper "MM-DD HH:MM UTC" formatı (saniye/ms auto-detect). `signal-grader.js` formasyon reasoning push'una detay satırı: `Formasyon: Çift Tepe (bearish, %75) | Tepe 1: 109.50 @ 04-20 00:13 UTC | Boyun çizgisi: 103.50 @ 04-20 10:13 UTC | Tepe 2: 110.50 @ 04-21 06:13 UTC | Hedef: 97.00`. `roleToTr()` Türkçe etiketleme. Smoke test geçti (sentetik Çift Tepe → 4 nokta zamanlı). 73/73 + 29/29 regression. | Claude |
| 2026-04-26 | 2.2 | **ADX yön (slope) hesabı eklendi** — Operatör fark etti: compute-regime.js trending teşhisinde `adxSlope >= 0` koşulu kullanıyor, ama scanner-engine `adxSlope: 0` hardcode geçiyordu (Faz 1 İter 1 TODO unutulmuş). Etkisi: ADX 30 ama düşüyor olsa bile sistem trending diyor → trend yorgunluğu yanlış tespit. Çözüm: `calcADX` `adxSeries` (son 5 değer) döndürüyor; `calcTechnicals` bu seriyi dağıtıyor; `scanner-engine.js` 3-bar fark `(adxSeries[-1] - adxSeries[-3]) / 3` slope hesabı. compute-regime artık gerçek slope bilgisini kullanıyor → trending_up/down doğrulaması "ADX > 25 VE yön yukarı/eşit" şeklinde çalışıyor. Test 8/8 (calc-adx) + 73/73 toplam + 29/29 root. Faz 1 raporu (May 2) öncesi devreye girdi → false-flip metrikleri slope-aware. | Claude + DeepSeek |
| 2026-04-26 | 2.1 | **Faz 2 v2.1: Rejim-aware minRR** — Patch (b) cleanup sonrası operatör 3 dashboard kartında (ETHXAU, ETHUSDC LONG, ETHUSDT.P LONG) kaynak sayısının arttığını ve B/C grade'lerin üretildiğini gözlemledi (cleanup başarılı). Yeni dar kalıp: ranging rejimde (ADX 9-14) iki sinyal B/C grade alabildi ama `minRR=1:2` katı eşiği IPTAL'e götürdü. Kök neden: minRR rejim-bağımsız (taxonomy v1.0 §2 zaten ranging'i ATR×1.5 SL ile farklılaştırmış). Çözüm: `REGIME_GATES`'e `minRR` alanı (ranging 1.5, trending 2.0, breakout 2.5, chaos/drift/closed null), `signal-grader.js` rejim-aware override (`regimeContext` varsa REGIME_GATES'ten oku). Etki: ranging ETHUSDC R:R 1:1.86 → GEÇER, ETHUSDT.P 1:1.60 → GEÇER. Test 70/70 + 29/29. May 2 raporu + outcome ile kalibre. | Claude + DeepSeek |
| 2026-04-26 | 2.0 | **Faz 2 Commit 5: Patch (b) cleanup** — KhanSaab full bypass mantığı `signal-grader.js`'den tamamen kaldırıldı. Operatör 2 dashboard kartında (ETHUSDC LONG IPTAL, BTCUSD SHORT BEKLE) "MACD/EMA/MA olmaması" sorununu işaret etti. Kök neden: Faz 0 patch (b) ADX<25'te `khanSaabForVotes=null` yapıp MACD/EMA/RSI/ADX/Volume/VWAP oylarını collectVotes'tan ÖNCE siliyordu → wrapper'a 4-7 oy ulaşıyordu (8-9 yerine) → wrapper'ın `REGIME_VOTE_WEIGHTS` rejim-aware suppression mantığı (ranging momentum 0.3, mean_reversion 1.5) **işlevsiz** kalıyordu. Çift mimari katman çakışması. Çözüm: 4 yerde patch (b) izleri silindi (`khanSaabDisabledFlag` tanımı, `_dirScore` ternary, `khanSaabEntry` ternary), `khanSaabForVotes = khanSaab` passthrough. Wrapper artık tüm 8-9 oyu alıp rejim-aware ağırlıklandırıyor. Test: 69/69 + 29/29. Shadow modda risk düşük; 24h gözlem temiz veriyle yeniden başlıyor. | Claude + DeepSeek |
| 2026-04-26 | 1.9 | **session-filter.js kaldırıldı** (çift sayım fix). Operatör UTC 23:30'da dashboard'da tüm kripto sinyallerinin "SESSION FILTER: Asya düşük-likidite penceresi (22:00-05:00 UTC)" gerekçesiyle BEKLE'ye düşürüldüğünü gözlemledi. Kök neden: 7 saatlik statik saat penceresi forex/emtia için geçerli olsa da kripto 24/7 piyasası için yanlış; ayrıca `low_vol_drift` rejimi (computeRegime) gerçek volatiliteyi zaten ölçüyor → çift sayım. Etki: kripto sinyallerinin günde ~%29'u (7/24) körlemesine reddediliyordu. Çözüm: `signal-grader.js`'den `checkSessionFilter` çağrısı kaldırıldı, `session-filter.js` silindi. Forex/emtia/BIST/ABD için `market-hours.js` zaten kapanış saatlerini tutuyor; kripto için Faz 1 wrapper `low_vol_drift` + `market_closed` rejimleri dinamik korumayı sağlıyor. | Claude + DeepSeek |
| 2026-04-26 | 1.8 | **Faz 2 başladı (Seçenek C)** — `regime-strategy.js` wrapper canlı (4 commit). REGIME_GATES (rejim-aware filtre eşikleri: ranging C/45/60), REGIME_VOTE_WEIGHTS (aile bazlı ağırlık çarpanları: ranging momentum 0.3, mean_reversion 1.5), REGIME_SL_MULT, vote suppression tablosu hayata geçti. `wrapper-mode.js` state file (`scanner/data/wrapper-mode.json`): default `shadow` (dispatch yok), 24h gözlem sonrası operator `POST /api/wrapper/mode {mode:"live", confirm:"I_REVIEWED_SHADOW_LOG"}` ile live'a geçer. `wrapper-decisions-YYYY-MM-DD.jsonl` audit trail. Scheduler dispatch wrapper-mode kontrolü ekledi (shadow ise log + return). signal-grader'a regimeContext + marketType + htfConfidence parametreleri (geri uyumlu, default null). 16/16 yeni test + 69/69 toplam. **REGIME_GATES sayıları geçici tahmin** — May 2 raporu + 5-10 günlük outcome verisiyle kalibre edilecek. | Claude + DeepSeek |
| 2026-04-25 | 1.7 | **Aşama A geçici köprü** — `alignment-filters.js` HTF-Barrier cap mantığına min-distance kuralı eklendi: cap edilen TP, SL mesafesinin en az 1.3 katı olmalı; aksi halde cap REDDEDILIR, orijinal TP'ler korunur. Operatör 5 dashboard kartında HTF-Barrier sıkışmasından kaynaklı toplu IPTAL gözlemledi (ETHUSDC 1:0.5, LINKUSDT 1:1.1, XRPUSDT 1:1.3, vb.). DeepSeek Unified Level Map önerdi → Faz 4'e ertelendi (rejim-aware için), bu arada saçma kısa TP üretmeyi engelleyen min-distance kuralı geçici köprü olarak devrede. `shouldRefuseBarrierCap()` helper export edildi (6/6 unit test). 24-48 saat gözlem sonrası ara raporda değerlendirilecek. | Claude + DeepSeek |
| 2026-04-25 | 1.6 | **Yeni Risk #18** — calcADX wilderRMA formul hatasi tespit edildi (operator dashboard'da ADX 137-243 değerleri gözlemledi). Wilder RMA klasik formul uygulandi (2 satir fix). Etki: shadow rejim verisi son haftada tamamen kirli (her sembol "trending"), SL multiplier "STRONG_TREND" overscale. Sanity test 5/5 + 47/47 toplam scanner test. Operator karariyla launchd timer kayma yok, server restart gerekli (canli ADX dogrulamasi icin). | Claude + DeepSeek |
